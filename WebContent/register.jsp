...
<script type="text/javascript">
  ...
  function _checkName(username){
	  ...
	  xhr.open("post","/estore/checkNameServlet",true);
	  ...
	  xhr.onreadystatechange=function(){
		  if(xhr.readyState==4&&xhr.status==200){
				...
				if(data==0){
					//用户可以注册
					document.getElementById("username_notice").innerHTML="<font color='green'>√</font>";
					controSub = true;
				}else{
					//用户不可以注册
					document.getElementById("username_notice").innerHTML="<font color='red'>×</font>";
					controSub =false;
				}
			}
		}
  };
  function _checkYzm(yzm){
	  var xhr=getXmlHttpRequest();
	  xhr.open("post","/estore/checkYzmServlet",true);
	  xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	  xhr.send("yzm="+yzm);
	  xhr.onreadystatechange=function(){
		  if(xhr.readyState==4&&xhr.status==200){
				...
				if(data==0){
					//用户可以注册
					document.getElementById("captcha_notice").innerHTML="<font color='green'>√</font>";
					controSub = true;
				}else{
					//用户不可以注册
					document.getElementById("captcha_notice").innerHTML="<font color='red'>×</font>";
					controSub =false;
				}
			}
		}
	...
	<div class="block block1">
		...
		<div class="usBox">
			<div class="usBox_1">
				...
				<form action="${root}/registerServlet" method="post" name="formUser"
					onsubmit="return (register()&&controSub);">
					<table width="100%" border="0" align="left" cellpadding="5"
						...
						<tr>
							...
							<td><input type="text" size="8" name="code" id="captcha"
								class="inputBg" onblur="check_captcha(this.value);_checkYzm(this.value);" /> <span style="color:#FF0000"
